...
13. [2026-05-07 후속 작업 — 라이브 배포 후 폴리싱](#13-2026-05-07-후속-작업--라이브-배포-후-폴리싱)
...
# 13. 2026-05-07 후속 작업 — 라이브 배포 후 폴리싱

5/6 시점의 배포 (vibloc26.com) 직후 같은 날 안에 진행된 보강. UI 인터랙션 완성도, Supabase 기반 멀티 사용자 동기화, 추천 엔진 다양성, 3D EQ 셰이더 폴리시. 7개 주제 클러스터, 40+ 커밋.

## 13.1 프로필 / 플레이리스트 인터랙션

**MyPage 헤더** — 96 × 96 원형 아바타 도입. 사진 없으면 이름 첫 글자 모노그램 (홈 우측 레일 ProfileRow와 같은 패턴, 사이즈만 28→96). 호버 시 `inset: 0` `rgba(0,0,0,0.45)` 스크림 + 28 px lucide 카메라 SVG `opacity 0→1` 160 ms 페이드. `role="button" tabIndex={0}` + Enter/Space → 숨겨진 file input 트리거. `FileReader.readAsDataURL` → `useAuthStore.updateUser({ avatarUrl: dataURL })`. 2 MB 캡 + 인라인 에러.

**이름 인라인 편집** — 32 × 32 원형 ✏ 버튼 → H1을 `<input>`로 교체 (display 폰트 그대로, 2 px ink 밑줄, autoFocus, max-width 480). Enter / Save 커밋 (빈 값 차단), Esc / Cancel 폐기.

**플레이리스트 커버** (`PlaylistDetailView` 120 × 120) — 같은 호버 카메라 + 업로드 어휘. `isMine === true`만 활성. 신규 데이터 모델 `BuildingPlaylistEntry.taggerPlaylistCovers: Record<taggerId, dataURL>` 추가, `loadFromStorage` 마이그레이션 포함. `getTaggerPlaylistCover()` getter + `setTaggerPlaylistCover()` setter. `getTopTaggers` 마지막 패스에서 user-cover 오버레이로 `g.customCoverUrl` 덮어씀 → 우측 레일 RankRow / FeaturedHero / PlaylistDetailView 모두 같은 진입점 통과해 즉시 반영. `BuildingPlaylist.tsx` MY PLAYLIST 카드의 36 × 36 모노그램도 공용 `PlaylistCover`로 교체 → 디테일뷰 업로드 즉시 카드 동기화.

**Stale broken-flag 버그** (`PlaylistCover.tsx`) — 한 번 404 났던 `customUrl`의 `customBroken=true` 상태가 prop 변경 후 유지되어 새 업로드가 모자이크 폴백되던 문제. `useEffect`로 `customUrl` / `usable[0]` 변경 시 자동 리셋.

**데이터 보존 가드** — `pinTrack` / `unpinTrack` / `setDescription` 세 writer 가 빌딩 entry 새로 만들 때 `{ tracks, description }`만 손으로 복사 → 사용자 커버 / 이름 / 노트 / 좋아요가 트랙 추가/제거나 설명 변경 시 모두 날아감. 세 writer 모두 `...entry` 스프레드 후 필요한 필드만 덮어씀. `unpinTrack` 자동 삭제 가드도 `description`만 보던 것에서 `hasCovers / hasNames / hasLikes`까지 확장.

## 13.2 인증 안전성 + 크로스-탭 동기화

**Supabase 토큰 리프레시 머지 가드** — `onAuthStateChange`는 토큰 자동 리프레시 / 페이지 포커스 / 새로고침 등에서 빈번히 발화하면서 `setSession(token, user)` 호출. user 전체 통째 덮어쓰는 기존 로직이 사용자 마이페이지 편집을 몇 초 후 롤백.
...
같은 `user.id`로 다시 호출 시 로컬 편집 우선 보존. 다른 id면 깨끗한 서버 프로필.

**서버 영속화** — `persistDisplayName()` 이름 변경 시 fire-and-forget `supabase.auth.updateUser({ data: { full_name, name } })` → user_metadata 영구 저장. 다른 기기 / 브라우저 / localStorage 클리어 후에도 유지. 아바타는 데이터 URL 크기로 user_metadata 한도 초과 → 로컬만 (Storage 버킷은 별도 슬라이스).

**크로스-탭 라이브 동기** — 같은 사이트 다중 탭 시나리오. localStorage 자체는 cross-tab `storage` 이벤트 broadcast하지만 zustand `persist` 와 buildingPlaylist 자체 store 모두 청취 안 함. 모듈 레벨 `window.addEventListener('storage', ...)` 4곳 추가:

| 모듈 | 키 | 액션 |
|---|---|---|
| `useAuthStore` | `vibloc-auth` | `useAuthStore.persist.rehydrate()` |
| `buildingPlaylist` | `vibloc.playlists.v1` | `loadFromStorage() + notify()` |
| `useDarkMode` | `vibloc.darkMode` | `setState({ darkMode })` |
| `i18n` | `vibloc.lang` | `setState({ lang })` |

결과: 한 탭에서 사진 / 이름 / 커버 / 다크 / 언어 변경 → 다른 모든 탭 즉시 반영, 새로고침 불필요.

## 13.3 멀티 사용자 백엔드 (Supabase + Realtime)

**Phase 1 — 좋아요** (`supabase/migrations/0001_shared_likes.sql`)
- `track_likes(building_id, track_id, user_id, liked_at)` — 3-tuple PK
- `playlist_likes(building_id, tagger_id::uuid, user_id, liked_at)` — 3-tuple PK
...
- 트랙 메타 denormalised → 읽기 시 join 불필요, iTunes API 다운에도 안전.
- 같은 RLS / Realtime 패턴.

**Client** (`lib/music/sharedLikes.ts`, `lib/music/sharedPins.ts`)
- 인메모리 캐시 (`Map<key, Set<userId>>` / `Map<buildingId, PinnedTrack[]>`).
- `boot*()`: 앱 부팅 시 전체 row hydrate + Realtime 채널 구독 (idempotent).
- `toggleSharedTrackLike()` / `toggleSharedPlaylistLike()` / `shareTrackPin()` / `shareTrackUnpin()`: 옵티미스틱 로컬 mutation → upsert/delete → 실패 시 롤백.
- UUID 가드 (dev-admin / synthetic id의 400 회피).
- Realtime postgres_changes 핸들러가 캐시 패치 + `notify()`.

**Bridge 패턴** (`buildingPlaylist.ts`)
...
- `mergeSharedLikesIntoStore()` / `mergeSharedPinsIntoStore()` — 서버 캐시를 entry.tracks[i].likedBy 배열 / entry.tracks 자체에 union → UI 코드 변경 0줄로 멀티 사용자 데이터 흡수.
...
**추천 캐시 + Sync peek** (`recommendForBuilding` / `peekRecommendation` / `invalidateRecommendation`)
- 빌딩 클릭마다 외부 API 4개 (Wikidata · iTunes · 영화 · 랜드마크) 매번 새로 호출하던 문제. `Map<area|buildingId|limit, {value, at}>` 10분 TTL, 64 LRU.
- `peekRecommendation()` 동기 조회 → `useState` 초기값으로 사용 → 캐시 히트 시 로딩 스피너 자체 안 뜸.
- 새로고침 버튼 → `invalidateRecommendation()` 호출 → 강제 재요청.

**다양성 + 인기 보장 + anti-repeat**
- RSS top 25 → **50** (풀 2배). `rssCandidates` `limit*2` → `limit*3`.
- `chartToppers` (top 5 by score before shuffle) `RecommendationResult`에 첨부.
- `pickWithOverlapLimit` 1슬롯을 chartToppers top 3 중 랜덤 1곡으로 예약 (장르 cap 우회) → 모든 패널에 최소 1개 차트 인기곡.
- 장르 cap `ceil(limit/2)` (5 결과 중 같은 장르 ≤ 2~3) → "전부 K-pop" 패턴 차단.
- `sessionShownTracks: Set<trackId>` — 세션 내 노출된 트랙 추적, candidate sort 시 미노출 우선 → 새로고침 = 진짜 새 곡.

**검색 결과 스크롤** — `AddTrackComposer` 결과 영역 전체를 `maxHeight: 360, overflowY: auto` 컨테이너로 감쌈. 입력창은 컨테이너 바깥 → 스크롤 중 sticky. 음악 검색 결과 cap 6 → 25. `SearchBar` (건물) 드롭다운 maxHeight 280 → `min(360px, 50vh)`.

**맵 기본값 + 도시 정렬** — `App.tsx` `?area=` 없을 때 fallback `shinjuku` → **`manhattan`** (랜딩 인트로 시네마틱 zoom-in과 일치). `CITY_AREAS` 키 순서: `manhattan, la, itaewon, gangnam, shinjuku, shibuya` (US → KR → JP). 좌측 사이드바 / 헤더 CityDropdown / useBuildingResolver 모두 `Object.keys(CITY_AREAS)` 사용 → 자동 적용. 랜딩 칩도 동일 순서.

## 13.5 레이아웃 정렬 + 모드 통일

**사이드바 정렬**
- 좌측 `FixedToolSidebar` 닫기 버튼이 row 아이콘과 6 px 어긋남 → header padding-left 16 → 24, 버튼 28×28 → 24×24, glyph 15 → 16. 광학 중심 x = 36 통일.
- 우측 `FixedQueueSidebar` ProfileRow 아바타 padding-left 8 → 12. 검색바 / MY PLAYLIST eyebrow / PopularRow 아트워크와 같은 x = 24 축에 정렬.

**다크 ↔ 라이브 상호 배타** (`App.tsx`)
...
**다크 포그 완화** (`PlateauScene.tsx`) — near 400 → 900 (+125%), far 2000 → 3400 (+70%), exponent 1.8 → 1.4. mid-range 빌딩 가독성 회복.

**Up Next 톤 분리** — 우측 레일 본체 `rgba(255,255,255,0.98)`와 동일해 떠 있는 패널 분간 어려움. token `paper` (#faf9f6) 0.96 알파 + ink 0.14 hairline + 그림자 강화 → "lifted card" 느낌. 다크: rgba(20,20,24,0.96) → rgba(28,28,32,0.96) 한 단계 elevation up.

## 13.6 3D 빌딩 EQ 셰이더 폴리시

**라이브 식별감 오버레이** (`getTopTaggers`) — PinnedTrack에 `taggerName`/`taggerAvatarUrl`이 pin time에 박혀 저장되어 사용자 닉네임 변경 후에도 옛 이름이 남던 문제. 그룹 빌드 마지막에 `_getUserIdentity()` 라이브 값으로 본인 그룹의 두 필드 오버레이.
...
- Per-column pulse: `mixedEnergy *= (0.92 + 0.08 * sine)` — 8% 미세 변조로 옆 컬럼끼리 desync, 음악 박자는 그대로 유지 (45% 강한 변조에서 8%로 줄임 — 박자 동기화 우선).

**볼륨 → 시각 보정** (`PreviewPlayer.tsx`) — `VISUAL_VOL_CEILING = 0.3`. 볼륨 ≤ 0.3 변화 없음, 그 이상은 `min(1, 0.3/userVolume)` 만큼 시각 신호 축소. 들리는 음량은 그대로, EQ 모양만 "낮은 볼륨의 다채로운 wave" 유지.

**선택 빌딩 효과 통일 (다크 ↔ 라이트)**
- `selGlow` 색감 청색 틴트 (`vec3(0.65, 0.82, 1.0)`) 제거 → 양 모드 동일 청백색 (`vec3(0.92, 0.96, 1.0)`).
- 본체 발광 / 림 라이트 강도 라이트와 동일 (0.06 / 0.40).
- Night blink + 전역 spectral blink 모두 선택 빌딩에서 비활성 (`* (1.0 - vIsSelected)`) → 선택 빌딩은 라이트 모드와 같은 차분한 면.

**Spectral blinks (전역)** — 다크 모드 + 음악 재생 중 (`uAudioActive * uDarkMode`) → 모든 빌딩 OFF 창문 ~8% 가 베이스 envelope (`uBeatLevel`) 기반으로 산발적으로 깜빡임. 선택 빌딩 제외. 도시에 살아있는 펄스감 부여.

## 13.7 성능

**`notify()` microtask 코얼레싱** (`buildingPlaylist.ts`) — 한 번의 pinTrack이 (로컬 mutation + 서버 mirror + Realtime echo + sharedPins refold) 4번의 notify cascade 유발. 각 listener가 `getTopTaggers` (O(전체 트랙수)) 재실행. `queueMicrotask`로 dedup → 같은 frame N번 notify → 1번 broadcast. `getTopTaggers` 호출 1/4로 감소.
...
라이브 검증: vibloc26.com Vercel 자동 배포 매 푸시마다 정상 빌드 확인.
...
├── main.tsx                     entry + Supabase shared-likes/pins boot
├── app/
│   └── router.tsx               react-router 설정
├── components/
│   ├── canvas/
│   │   ├── PlateauScene.tsx     R3F Canvas + 카메라 + fog + 효과
│   │   ├── OSMCity.tsx          OSM 빌딩 geometry + EQ 셰이더
...
│   │       ├── BuildingPlaylist.tsx        MY PLAYLIST (커버 동기)
│   │       ├── RecommendedList.tsx         AI PICKS (캐시 + 새로고침)
│   │       ├── AddTrackComposer.tsx        검색 컴포저 (스크롤 well)
│   │       ├── PlaylistDetailView.tsx      플레이리스트 상세 (커버 업로드)
│   │       ├── PlaylistCover.tsx           2×2 모자이크 + broken-flag 리셋
│   │       ├── PreviewPlayer.tsx           오디오 + 큐 + 볼륨 보정
│   │       └── MarqueeText.tsx             overflow 마키
├── features/
│   ├── auth/                    Supabase auth + 토큰 머지 가드 + persist 동기
│   └── profile/                 빌딩 resolver, my-music
├── lib/
│   ├── geo/                     OSM loader, Plus Code
│   ├── music/                   iTunes wrapper, recommendation engine
│   │   ├── sharedLikes.ts       Supabase 멀티 사용자 좋아요 (Realtime)
│   │   ├── sharedPins.ts        Supabase 멀티 사용자 핀 (Realtime)
│   │   └── buildingPlaylist.ts  + bridge 훅 (shared* 미러링)
│   ├── ui/
│   │   ├── tokens.ts            COLOR / FONT / SPACE / RADIUS
│   │   └── toast.ts             toast pub/sub store
│   ├── app/
│   │   ├── i18n.ts              + cross-tab storage 동기
│   │   └── useDarkMode.ts       + cross-tab storage 동기
│   └── share/                   Apple Music 딥링크
├── pages/
│   ├── landing/LandingPage.tsx  랜딩 (시네마틱 인트로, Manhattan 기본)
│   ├── auth/                    로그인 / 회원가입
│   ├── mypage/                  내 페이지 (아바타 + 인라인 이름 편집)
│   └── share/                   공유 프리뷰 페이지
├── stores/                      zustand stores (time, weather, building)
└── (root)
    └── supabase/
        ├── README.md            마이그레이션 적용 절차
        └── migrations/
            ├── 0001_shared_likes.sql
            └── 0002_shared_pins.sql
```

---

**문서 마지막 수정**: 2026-05-07
**구성**: 12개 본문 섹션 + §13 (배포 후 폴리싱 7개 클러스터) + Appendix A·B
**다음 업데이트**: Supabase Storage 버킷 기반 커버 이미지 server-side hosting (Phase 3)
